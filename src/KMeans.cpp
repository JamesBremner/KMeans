...
void KMeans::Assign()
{
    ...
    for (auto &cl : myLocations)
    {
        ...
        //cout << "assigned " << cl->text() << " to cluster " << myClusters[closest_cluster].text() << "\n";
    }
}
...
void KMeans::MoveClustersToMean()
{
    for (int ks = 0; ks < myClusterCount; ks++)
    {
        ...
        if (count)
        {
            // Average of all data points assigned
            A = A / count;

            // cout << "cluster at " << myClusters[ks].center().text()
            //      << " has " << count 
            //      << " moving to "  << A.text() << "\n";

            // move cluster to average
            myClusters[ks].move(A);
        }
    }
}
